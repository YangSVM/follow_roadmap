...
class PurePursuit():
    ...
    def compute_cmd(self, msg):
        ...
        if vel_cmd == 0:
            rospy.logwarn('pp roadpoint stop!')
    ...
    def get_preview_roadpoint(self, local_traj_xy, id_current, preview_distance, posture):
        position = local_traj_xy[id_current,:]#posture[:2]          # 距离自车最近点
        # 距离自车距离再增加 preview_distance
        ...
        # 解方程 abs(distance_real - preview_distance) =0
    ...
    def get_local_trajectory(self):
        ...
        with open(roadMapPath, "r") as f:
            for line in f.readlines():
                ...
                roadpoints.y = float(line[1])
                roadpoints.x = float(line[2])
