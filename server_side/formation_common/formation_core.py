...
def judge_formation_end(poses:np.ndarray, csps: List[Spline2D])->int:
    ''' 判断是否已经完成给定的编队
    '''
    isDones = [True for i in range(poses.shape[0])]
    # 使用绝对距离判断
    # d_thre = 1
    # for i, csp in enumerate(csps):
    #     pose = poses[i, :2]
    #     csp_end = np.array([csp.sx.y[-1], csp.sy.y[-1]])
    #     d = np.linalg.norm(pose-csp_end)
    #     if d>d_thre:
    #         isDones[i] = False
    #         break
    ...
    for i, csp in enumerate(csps):
        pose = poses[i, :2]
        s0,  _ = cartesian2frenet(pose[0], pose[1], csp)
        if abs(s0 - csp.s[-1]) > 1:
            isDones[i] = False

    rospy.loginfo('编队完成标志：'+str(isDones))
    return all(isDones)


def judge_formation_stage(s: List, formation_begin_s: List, ds=5)->np.ndarray:
    '''根据状态进行划分
    stages: 过渡阶段为负数。稳态阶段为自然数。
    '''
    s = np.array(s)
    n_stages = len(formation_begin_s)

    stages = np.ones(s.shape, dtype=int)*(n_stages-1)
    
    stages[s<=formation_begin_s[0]] = 0

    for i in  range(n_stages):
        # 逆序赋值
        j = n_stages - i - 1
        stages[s<formation_begin_s[j]+ds[j]] = -j
        stages[s<formation_begin_s[j]] = j-1
    
    return stages

def assgin_traj(poses: np.ndarray, csps:List[Spline2D]):
    ''' 分配轨迹。cost为到该直线的距离。
    '''
    n_car = poses.shape[0]

    cost_matrix  = np.zeros([n_car, n_car])
    for i in range(n_car):
        # 每辆车
        for j in range(n_car):
            # 每条轨迹
            _,d = cartesian2frenet(poses[i, 0], poses[i, 1], csps[j])
            cost_matrix[i, j] = abs(d)
    
    _, matches_j = linear_sum_assignment(cost_matrix)
    return matches_j
...
def Assign(pos_start, pos_end):
    ...
    Params:
        pos_start, pos_end: np.ndarray. [n_car, 2]
    '''
    x_start = pos_start[:, 0]
    y_start = pos_start[:, 1]
    x_end = pos_end[:, 0]
    ...
    # matches_i 是已经排好序的结果。[i, j] 表示选中的cost。
    ...
    return pos_end_assign,  matches_j
