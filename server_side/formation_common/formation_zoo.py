...
def gen_formation(center:np.ndarray, theta, n_car:int, d_car:float, formation_type: FormationType, dim=2):
    '''
    Params:
        theta: 弧度制.队形朝向
    '''
    ...
    if dim == 3:
        pos_formation_pro = np.ones([n_car, 3]) * theta
        pos_formation_pro[:, :2] = pos_formation
        pos_formation = pos_formation_pro
