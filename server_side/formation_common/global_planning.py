...
def generate_route_with_formation(center_line: np.ndarray, formation_begin_s: List, formation_types: List[FormationType],
n_car: int, d_car: float =2.5,  ds_trans=[])-> Tuple[Spline2D, List[Spline2D], np.ndarray, np.ndarray]:
    '''
    ...
    formation_begin_s = np.array(formation_begin_s)
    # 计算队形的起始位姿。以及辅助的s坐标
    n_stage = formation_begin_s.shape[0]
    formation_begin_pos = np.zeros([n_stage, 3])       # x,y,yaw(弧度制)

    formation_begin_s = formation_begin_s
    for i, s in enumerate(formation_begin_s):
        if s>global_csp.s[-1]:
    ...
    # 匈牙利算法分配轨迹，并且生成多条csp
    # 目标分配
    matches = np.zeros([n_stage-1, n_car], dtype=int)        # 目标分配结果

    for i_stage in range(0, n_stage-1):
        _, matches[i_stage, ...] = Assign(formation_poses[2*i_stage, :], formation_poses[2*i_stage+2, :])
        if i_stage == 0:
            matches[i_stage, :] = matches
        else:
    ...
    for i in range(n_s):
        ...
        # 根据目标分配结果重组局部轨迹
        if i_stage > 0:
                ind_pos[i, ...] = ind_pos[i, matches[i_stage-1, :], :]
...
def dubins_pf_pro(car_poses, ctrl_points=[], r=r_pix, step_size=0.1, len_end_line = 1):
    ...
    # # 在前面加一条短直线
    # t1 = -car_poses[0, 2]
    # line_pre = gen_line(car_poses[0, :2]+2*np.array([cos(t1), sin(t1)]), car_poses[0, :2], dim=3)
    # trajs.extend(line_pre)
