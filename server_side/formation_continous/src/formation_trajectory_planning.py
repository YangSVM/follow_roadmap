...
class FormationTrajPlanning(MATrajPlanningBase):
    ...
    def initial(self, pose_states: List[Pose]):
        ...
        # matches更改
        for i_stage in range(0, len(self.fs.matches)):
    ...
    def transition_control(self, fps, poses, s_leader):
        ...
        formation_np = formation_np[self.fs.matches[i_stage-1, :], :]
...
def cut_csp(csp:Spline2D, s0, delta_s=10, v_const = TARGET_SPEED):
    ...
    path_x, path_y, path_v = np.ones(n_points), np.ones(n_points), np.ones(n_points)*v_const
