...
# 
def tapf(begin_formation, end_formation, matches=[], r=r_pix):
    ''' task asignment and path finding。使用匈牙利算法和dubin pf pro规划轨迹
    Params:
        begin_formation: [n_car, 3]. 每辆车的起始位姿
        end_formation: [n_car, 3]. 每辆车的结束位姿
        matches: 可以指定给定任务规划。
    Returns:
        routes: list of list. n_car* n_path_points. 每辆车的轨迹
        matches: 匈牙利算法的规划结果
    '''
    if len(matches) == 0:
        _, matches = Assign(begin_formation[:, :2], end_formation[:, :2])
    # 分配目标位置
    end_formation = end_formation[matches, :]
    n_car = begin_formation.shape[0]
    routes = []
    for i_car in range(n_car):
        paths = dubins_pf_pro(np.array([begin_formation[i_car, :], end_formation[i_car, :]]), r=r)
        routes.append(paths)

    return routes, np.array([matches])
...
def build_up():
    ...
    matches = np.arange((n_stage-1)*n_pix).reshape([n_stage-1, n_pix])
    ...
    matches[0, :] = np.arange(n_pix)
    pos_stage[1, ...] = pos_stage[1, matches[0, :], :]
...
# task: 侦查后返回。编队队形变换。
def after_search():
    pix_points, jc_poses, tp_pos = load_critial_poses(i_pix=[2,3])
    
    center_line = dubins_pf_pro(pix_points)
    ci1 = dubins.shortest_path(pix_points[0, :], pix_points[1, :], r_pix)
    _, s1 = ci1.sample_many(step_size)
    ...
    formation_begin_ls, formation_type_ls, ds_trans_ls = load_formation_turnning_config(car_colors, s1[-1])
...
def battle_part2():
    # 打击阶段2：三角队形走到打击位置
    # 用from global存在断轨迹。采用from individual.
    pix_points, jc_poses, tp_pos = load_critial_poses(i_pix=[3,4], i_jc=[1,2])
    mid_point = (pix_points[0, :] + pix_points[1, :])/2           # 走到中间位置队形进行变换
    mid_point[2] = pix_points[1, 2]
...
def battle_part2_ind():
    ...
    pix_points[0, :] = mid_point
    car_colors = [[0 for i in range(n_pix)], [1]]
    end_formations = gen_formation(pix_points[1, :2], pix_points[1, 2], n_pix, d_car, FormationType.Triangle, dim=3)

    global last_end_poses
    pix_routes, matches = tapf(np.array(last_end_poses), end_formations)

    formation_begin_s = [0, 0.1]
    formation_types = [FormationType.Line,  FormationType.Triangle]
    ds_trans = [0, -1]
    fs = FormationStage.from_individual(pix_routes, formation_begin_s, formation_types, ds_trans, matches)
    jc_poses[0, :2] = (jc_poses[0, :2] + jc_poses[1, :2])/2             # 起始位置改成中点
    jc_poses[0, 2] =  jc_poses[1, 2]

    jc_route = dubins_pf_pro(jc_poses)
    fs_jc = FormationStage.from_global(jc_route, 1, formation_begin_s, formation_types, d_car, ds_trans)
...
def pursuit():
    ...
    matches = np.arange(n_pix).reshape([1, n_pix])
...
def load_task_configs():
    ...
    for i in range(task, n_task):
        if i ==0:
            ...
            car_colors , fss = battle_part2_ind()
        elif i==5:
