...
from adm2ctrl_pb2_grpc import DtoCStub
import formation_common
...
def vehicle_update(msg, vehicleState: VehicleState):
    ...
    v0 = vehicleState.v
    a = (v - v0)/ts
    steer0 = vehicleState.steer
    steer = steer0 + omega*ts
...
def simulation():
    ...
    # 车辆起始位姿
    initial_pose = [0, -10, 90]
    vehicleState = VehicleState(initial_pose[0], initial_pose[1], initial_pose[3] *np.pi /180)

    # 输入控制量
    rospy.Subscriber('control_cmd',String, vehicle_update, vehicleState)
